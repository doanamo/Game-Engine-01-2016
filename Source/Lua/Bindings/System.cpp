...
System::InputState* InputState::Check(lua_State* state, int index)
{
...
int InputState::IsKeyDown(lua_State* state)
{
    Assert(state != nullptr);
...
int InputState::IsKeyUp(lua_State* state)
{
    Assert(state != nullptr);
...
void InputState::Register(Lua::State& state, Context& context)
{
    ...
    *pointer = context.inputState;
    ...
    lua_pushcfunction(state, InputState::IsKeyDown);
    lua_setfield(state, -2, "IsKeyDown");

    lua_pushcfunction(state, InputState::IsKeyUp);
...
void KeyboardKeys::Register(Lua::State& state, Context& context)
{
    Assert(state.IsValid());
